...
/*
	...
	File: king-include/king-db-post-update.php
...
*/
	...
	function qa_db_post_set_content($postid, $title, $content, $format, $tagstring, $notify, $lastuserid=null, $lastip=null, $updatetype=QA_UPDATE_CONTENT, $name=null, $pcontent=null, $nsfw=null)
/*
...
*/
	{
		if (isset($lastuserid) || isset($lastip)) // use COALESCE() for name since $name=null means it should not be modified (for backwards compatibility)
			qa_db_query_sub(
				'UPDATE ^posts SET title=$, content=$, format=$, tags=$, name=COALESCE($, name), notify=$, updated=NOW(), updatetype=$, lastuserid=$, lastip=INET_ATON($), pcontent=$, nsfw=$ WHERE postid=#',
				$title, $content, $format, $tagstring, $name, $notify, $updatetype, $lastuserid, $lastip, $pcontent, $nsfw, $postid
			);
		else
			qa_db_query_sub(
				'UPDATE ^posts SET title=$, content=$, format=$, tags=$, name=COALESCE($, name), notify=$, pcontent=$, nsfw=$ WHERE postid=#',
				$title, $content, $format, $tagstring, $name, $notify, $pcontent, $nsfw, $postid
			);
	}
