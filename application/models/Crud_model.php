...
class Crud_model extends CI_Model
{
    ...
    public function get_status_shorts($status = "")
    {
        $multi_instructor_course_ids = $this->multi_instructor_course_ids_for_an_instructor($this->session->userdata('user_id'));
    ...
    {
        if (isset($_FILES['favicon1']) && $_FILES['favicon1']['name'] != "") {
            unlink('uploads/system/' . get_frontend_settings('favicon1'));
            $data['value'] = md5(rand(1000, 100000)) . '.png';
            $this->db->where('key', 'favicon1');
            $this->db->update('frontend_settings', $data);
            move_uploaded_file($_FILES['favicon1']['tmp_name'], 'uploads/system/' . $data['value']);
        }
        //move_uploaded_file($_FILES['favicon1']['tmp_name'], 'uploads/system/favicon.png');
    }
